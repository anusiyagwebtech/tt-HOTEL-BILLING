...
if (isset($_REQUEST['submit'])) {
    ...
    $imag = strtolower($_FILES["user_profile_photo"]["name"]);
    $pimage = get_users('user_profile_photo', $getid);

    if ($imag) {
        if ($pimage != '') {
            unlink("../../img/users/" . $pimage);
            unlink("../../img/users/thump/" . $pimage);
        }
        $main = $_FILES['user_profile_photo']['name'];
