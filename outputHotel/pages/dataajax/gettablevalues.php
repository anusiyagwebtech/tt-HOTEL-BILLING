...
if ($_REQUEST['types'] == 'financecustomertbale') {
    $aColumns = array('id','name', 'object','mobileno');
    $sIndexColumn = "id";
    $editpage = ($_REQUEST['db_table_for'] == 'live') ? "editbill" : "editbill";
    $sTable = "customer";
}
...
while ($aRow = $rResult->fetch(PDO::FETCH_ASSOC)) {
    ...
    for ($i = 0; $i < count($aColumns1); $i++) {
		...
		else if ($_REQUEST['types'] == 'purchasetable') {
            if ($aColumns1[$i] == $sIndexColumn) {
                $row[] = $k;
    ...
    } elseif (($_REQUEST['types'] != 'storeorder') &&  ($_REQUEST['types'] != 'onlineorder') &&  ($_REQUEST['types'] != 'objecttable1') &&  ($_REQUEST['types'] != 'expensetable') && ($_REQUEST['types'] != 'unittable') && ($_REQUEST['types'] != 'subcategorytable') && ($_REQUEST['types'] != 'categoriestable') && ($_REQUEST['types'] != 'hotelusertable') && ($_REQUEST['types'] != 'objecttable') && (($_REQUEST['types'] != 'financecustomertbale') && ($_REQUEST['types'] != 'returntable') && ($_REQUEST['types'] != 'billtable') && ($_REQUEST['types'] != 'bankstatustable') && ($_REQUEST['types'] != 'suppliertable')  && ($_REQUEST['types'] != 'purchasetable') && ($_REQUEST['types'] != 'salestable') && ($_REQUEST['types'] != 'silverobjecttable'))) {
