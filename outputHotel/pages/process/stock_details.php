                        ...
                        <table id="normalexamples" class="table table-bordered table-striped">
                            <thead>
                                <tr align="center" style="font-size:19px;">
                                    ...
                                    <th style="width:8%">Used Stock</th>
									 <th style="width:8%">Available Stock</th>
                                </tr>
                            </thead>
<?php  
    global $db;

$message1 = $db->prepare("SELECT * FROM `stock_product` WHERE `id`!='0'");	
$m=1;
$message1->execute();
while($message = $message1->fetch(PDO::FETCH_ASSOC)) {

$Tostkqry = FETCH_all("SELECT sum(`pquantity`) AS `totqty` FROM `purchase_object_detail` WHERE `productid`=?", $message['id']);
$usedqtyqry = FETCH_all("SELECT sum(`quantity`) AS `totqty` FROM `stock_usage` WHERE `product`=?", $message['id']);
$remqty=$Tostkqry['totqty'] - $usedqtyqry['totqty'];

?> 
<tr>
<td><?php echo $m; ?></td>
<td><?php echo getobject1('objectname',$message['id']); ?></td>
