...
$supplier = FETCH_all("SELECT * FROM `online_store_order` WHERE `id`=?", $_REQUEST['id']);
?>
  <div id="invoice-POS">
    ...
    <center id="top">
      ...
      <!--  <img src="http://localhost:8080/westernfinance/pages/profile/image/1616745309.png" height="50" width="50"><br>-->
		...
		<h2>Peter's Boat<br><h2>
		<h4>No, 248/2 (Opp.to SP Office) Near River City Vegetable Shop<br>Alagar Kovil Main Road, Surveyor Colony, Madurai - 625 007</h4>
    ...
    <div id="mid">
      <div id="table">
		...
		<td align="right" valign="top"><strong>Date :</strong> <?php echo $supplier['date']; ?><br><br>
		<strong><?php if($supplier['order_type']=='1') { echo "Vendor Order"; } else { echo "Phone Order"; } ?></strong>
		</td>
		</tr>
		</table>
      </div>
...
<?php
 $sno =1;
    $tot_amnt=0;
    $object_detail = $db->prepare("SELECT * FROM `online_store_order_deatils` WHERE `object_id`= ?");
...
<br>
   <div id="mid">
      <div id="table" align="right" cellpadding="8" cellspacing="7">
        <table width="70%">
